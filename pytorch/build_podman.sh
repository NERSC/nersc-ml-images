...
NERSC_TAG_VERSION=${NERSC_TAG_VERSION:-01}
NERSC_IMAGE_NAME=${NERSC_IMAGE_NAME:-nersc/pytorch}
...
PULL_SHIFTER=${PULL_SHIFTER:-false}


# Build the image with podman-hpc
podman-hpc build --build-arg nvc_tag=$NVC_TAG-py3 -t $NERSC_IMAGE -f Dockerfile .
